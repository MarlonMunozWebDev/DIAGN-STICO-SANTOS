...
<div class="map all_pad">
	<div class="container">
		<h3 class="title agile">Visítanos</h3>
		<iframe src="https://www.google.com/maps/d/embed?mid=1i0H9UmznqJH8gKB3yJJ6FnnxmYA" width="640" height="480"></iframe>		<div class="contact-grids w3layouts">
			<h3 class="title">Contáctanos<span></span></h3>

			<?php 
			include_once 'dashboard/conexion.php';
			$sql_leer = 'SELECT * FROM generales';
			$sentencia_leer = $pdo->prepare($sql_leer);
			$sentencia_leer->execute();
			$generales = $sentencia_leer->FetchAll();
			?>
				...
				<div class="col-md-4 contact-grid w3 text-center animated wow slideInRight" data-wow-delay=".5s">
					<div class="contact-grid3 w3l">
						...
						<h4>Correo</h4>
						<p><a href="mailto:info@example.com"><?php echo $general['correo_electronico']?></a></p>
					</div>
				</div>
				<?php  endforeach ?>
...
<div class="contact_w3agile">
	<?php 
		if($_POST) {
			...
			$contacto_fecha = $_POST['fecha'];
			$agregar_mensaje = 'INSERT INTO mensajes (nombre,correo,telefono,mensaje,fecha) VALUES (?,?,?,?,?)';
			$sentencia_mensaje = $pdo->prepare($agregar_mensaje);
			$sentencia_mensaje->execute(array($contacto_nombre,$contacto_correo,$contacto_telefono,$contacto_mensaje,$contacto_fecha));
			$sentencia_mensaje = null;
			$pdo = null;
		}
	?>
	<div class="container">
		<h2 class="title agileits-w3layouts">Consúltanos</h2>
		<div class="strip"></div>
		<ul>
		...
		<form method="post">
			<input type="text" value="Nombre y Apellidos*" name="nombre" minlength="14">
			<input type="email" value="Correo*" name="correo">
			<input type="text" value="Teléfono" name="telefono">
			<input type="hidden" name="fecha" step="1" min="2020-01-01" max="2020-12-31" value="<?php echo date("Y-m-d");?>">
			
			<textarea name="mensaje" minlength="400">Mensaje*</textarea>
